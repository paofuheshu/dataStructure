...
public class ActiveDirectory {
   ...
   *  用于验证给定用户名密码和域名的用户。提供用于标识特定 Active Directory 服务器的选项。
   */
   ...
   *  or "ACME\peter").
   *  用于检查用户名是否有效。 @param username 要验证的用户名（例如“peter”、“peter@acme.com”或“ACME\peter”）。
   */
   ...
   */
    public static class User {
       ...
       *  Example:
          ...
          用于更改用户密码。如果域控制器未启用 LDAPS，则引发 IOException。
          trustAllCerts 如果为真，则绕过所有证书和主机名验证。
          如果为 false，请确保在调用此方法之前已将 LDAPS 证书导入到信任库并获取源。
          示例：String keystore = "usrjavajdk1.5.0_01jrelibsecuritycacerts";
          System.setProperty("javax.net.ssl.trustStore",keystore)
       */
